...
const Login = () => {
    ...
    const isValid=()=>{
       const error= validation(email?.current?.value,password?.current?.value,name?.current?.value,islogin)
  ...
  return (
    ...
    <div >
       ...
       <form onSubmit={(e)=>e.preventDefault()} className='absolute w-3/12  bg-black my-56 mx-auto left-0 right-0 p-12 text-white rounded-lg opacity-85'>
          <h1 className='font-bold'>{!islogin?"Sign In":"Sign Up"}</h1>
        ...
        <p className='text-red-500 font-bold '>{errmessage}</p>
          <button 
          className='w-full bg-red-700 my-4 p-4 rounded-lg' 
          onClick={isValid}>
          ...
          <button 
          className='w-full bg-gray-700 my-4 p-4 rounded-lg'>
            use sign in code
         </button>
